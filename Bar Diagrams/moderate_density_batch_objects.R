...
# Filter data for moderate density and the specified types
filtered_data <- data %>%
  filter(density == "moderate" & type %in% c("without", "all_ped", "with", "all"))
...
speed_plot <- ggplot(stats, aes(x = type, y = speed_avg, fill = type)) +
  ...
  scale_fill_manual(values = c("without" = "red", "all_ped" = "blue", "with" = "green", "all" = "purple"))
